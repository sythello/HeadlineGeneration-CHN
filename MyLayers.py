...
# Input shape = [(None, timesteps, input_dim), (None, timesteps, input_dim), (None, timesteps, input_dim)]
# 0 ==> input
# 1 ==> references (former outputs) (when training, it's the labels)
# (generate the whole sequence, but only one word is useful, which is extracted in the model)
# 2 ==> attention
# Not implemented yet!

class Attention_Ref_GRU(Layer):
    '''
    units: Positive integer, dimensionality of the output space.
    activation: Activation function to use
        (see [activations](../activations.md)).
    ...
    def build(self, input_shape):
        if not isinstance(input_shape, list):
            raise ValueError('An Attention_GRU layer takes input as [input_seq, output_seq, att_seq]')

        batch_size = input_shape[0][0]
        self.input_dim = input_shape[0][2]
        self.output_dim = input_shape[1][2]
        self.att_dim = input_shape[2][2]
        self.input_spec = [InputSpec(shape=(batch_size, None, self.input_dim)), InputSpec(shape=(batch_size, None, self.att_dim))]
        self.state_spec = InputSpec(shape=(batch_size, self.units))
